...
def main():
    ...
    # dMz / dbeta
    #
    # axis 1 corresponds to beta

    dMz_dbeta_grid = np.gradient(
        Mz_grid,
        d_beta,
        axis=1
    )
    ...
    # dMz / ddelta
    #
    # axis 0 corresponds to delta

    dMz_ddelta_grid = np.gradient(
        Mz_grid,
        d_delta,
        axis=0
    )
    ...
    # ===================================
    # CSR PLOTTING VERSION
    # ===================================

    # Raw CSR_grid is preserved.
    #
    # CSR_plot is only used to prevent
    # singularities from destroying the
    # visualization.
    ...
    ax.set_xlabel(
        "Lateral Acceleration $A_y$ (g)",
        labelpad=12
    )

    ax.set_ylabel(
        "Yaw Moment $M_z$ (ft-lb)",
        labelpad=12
    )
    ...
    print(
        f"dMz/dbeta = "
        f"{dMz_dbeta * vp.NM2FTLB:.2f} "
        f"ft-lbs/rad"
    )

    print(
        f"dMz/ddelta = "
        f"{dMz_ddelta * vp.NM2FTLB:.2f} "
        f"ft-lbs/rad"
    )

    print(
        f"Control / Stability Ratio = "
        f"{CSR_center:.2f}"
    )
    ...
    print(
        f"MZ MAX = "
        f"{np.max(Mz_ftlb_grid):.2f} "
        f"ft-lbs"
    )

    print(
        f"MZ MIN = "
        f"{np.min(Mz_ftlb_grid):.2f} "
        f"ft-lbs"
    )

    print(
        f"AY MAX = "
        f"{np.max(Ay_grid):.2f} g's"
    )

    print(
        f"AY MIN = "
        f"{np.min(Ay_grid):.2f} g's"
    )
